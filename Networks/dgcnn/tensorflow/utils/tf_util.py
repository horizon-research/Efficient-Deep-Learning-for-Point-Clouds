...
def fully_connected(inputs,
  ...
  with tf.variable_scope(scope) as sc:
    ...
    print("(fc in", scope,") ", outputs.shape)
...
def pairwise_distance(point_cloud):
  ...
  """
  with tf.name_scope("pair_distance"):
...
def knn(adj_matrix, k=20):
  ...
  with tf.name_scope("knn"):
      ...
      print("(tf_util) top_k - input size", neg_adj.shape)
      print("(tf_util) top_k - nn indices", nn_idx.shape)
      print("(tf_util) top_k - dist values", _.shape)
  return nn_idx
...
def get_edge_feature(point_cloud, nn_idx, k=20):
  ...
  with tf.name_scope("get_edge_feature"):
      og_batch_size = point_cloud.get_shape().as_list()[0]
      point_cloud = tf.squeeze(point_cloud)
      if og_batch_size == 1:
        point_cloud = tf.expand_dims(point_cloud, 0)

      print("get_edge_feature: point_cloud", point_cloud.shape)
      ...
      print("get_edge_feature: idx_", idx_.shape)
      idx_ = tf.reshape(idx_, [batch_size, 1, 1]) 
      print("get_edge_feature: idx_", idx_.shape)

      point_cloud_flat = tf.reshape(point_cloud, [-1, num_dims])
      print("get_edge_feature: point_cloud_flat", point_cloud_flat.shape)
      
      point_cloud_neighbors = tf.gather(point_cloud_flat, nn_idx+idx_)
      print("get_edge_feature: point_cloud_neighbors", point_cloud_neighbors.shape)

      point_cloud_central = tf.expand_dims(point_cloud_central, axis=-2)
      print("get_edge_feature: point_cloud_central (expand_dims)", point_cloud_central.shape)
      
      point_cloud_central = tf.tile(point_cloud_central, [1, 1, k, 1])
      print("get_edge_feature: point_cloud_central (tile)", point_cloud_central.shape)
      print("get_edge_feature: tile == duplicate")
      edge_feature = tf.concat([point_cloud_central, point_cloud_neighbors-point_cloud_central], axis=-1)
      # [points, neighbor] for k times
      # points dupliated many times

      '''
      edge_feature = tf.Print(edge_feature, [edge_feature[0][0][0]], message="edge feature", summarize=-1)
      edge_feature = tf.Print(edge_feature, [edge_feature[0][0][1]], message="edge feature", summarize=-1)
      edge_feature = tf.Print(edge_feature, [edge_feature[0][0][2]], message="edge feature", summarize=-1)
      edge_feature = tf.Print(edge_feature, [edge_feature[0][0][3]], message="edge feature", summarize=-1)
      edge_feature = tf.Print(edge_feature, [edge_feature[0][0][4]], message="edge feature", summarize=-1)
      '''
      print("get_edge_feature: edge feature", edge_feature.shape)
