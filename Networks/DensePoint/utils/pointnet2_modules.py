...
class _PointnetSAModuleBase_new(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        for i in range(len(self.groupers)):
            new_features = torch.cat([features, xyz.transpose(1, 2).contiguous()], dim = 1)
            ...
            # aggregation:
            if self.flag_aggregation:
                self.groupers[i](xyz, new_xyz, new_features) # aggregation + maxpooling, central normalization here
            all_features += new_features
        return new_xyz, all_features


class PointnetSAModuleMSG_new(_PointnetSAModuleBase_new):
    ...
    def __init__(
        ...
        self.pool = pool
        self.npoint = npoint
        self.flag_aggregation = flag_aggregation
        ...
        for i in range(len(radii)):
            ...
            self.groupers.append(
                pointnet2_utils.QueryAndGroup_new(radius, nsample, use_xyz=use_xyz)
            )
            mlp_spec = mlps[i]
            if use_xyz:
                mlp_spec[0] += 3
